...
function UploadButton({ onUploaded }: { onUploaded: (url: string) => void }) {
  ...
  return <label className="upload-button">{loading ? <AdminIcon spin>o</AdminIcon> : <AdminIcon>^</AdminIcon>}<span>{loading ? "Uploading..." : "Upload image"}</span><input type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => upload(event.target.files?.[0])} /></label>;
}
...
export function AdminClient({ authenticated, initialContent }: { authenticated: boolean; initialContent: PortfolioContent | null }) {
  ...
  return <main className="admin-page">
    <header className="admin-header"><div><p>MF CONTENT MANAGER</p><h1>Portfolio backoffice</h1></div><div><a href="/" target="_blank">View website</a><Button variant="outline" onClick={logout}><AdminIcon>&gt;</AdminIcon> Sign out</Button><Button onClick={save} disabled={saving}>{saving ? <AdminIcon spin>o</AdminIcon> : <AdminIcon>#</AdminIcon>}{saving ? "Saving..." : "Save changes"}</Button></div></header>
    ...
    <Tabs defaultValue="general" className="admin-tabs">
      ...
      <TabsContent value="about"><section className="admin-card"><h2>About me</h2><Field label="Introduction" value={content.about.intro} multiline onChange={(v) => updateAbout("intro", v)} /><Field label="Professional background" value={content.about.description} multiline onChange={(v) => updateAbout("description", v)} /><h3>Traits</h3><div className="repeat-list">{content.about.traits.map((trait, index) => <div key={index}><Input value={trait} onChange={(e) => { const traits = [...content.about.traits]; traits[index] = e.target.value; setContent({ ...content, about: { ...content.about, traits } }); }} /><Button variant="ghost" size="icon" aria-label="Remove trait" onClick={() => setContent({ ...content, about: { ...content.about, traits: content.about.traits.filter((_, i) => i !== index) } })}><AdminIcon>x</AdminIcon></Button></div>)}</div><Button variant="outline" onClick={() => setContent({ ...content, about: { ...content.about, traits: [...content.about.traits, "New trait"] } })}><AdminIcon>+</AdminIcon> Add trait</Button></section></TabsContent>
      <TabsContent value="services"><section className="admin-card"><div className="card-heading"><h2>Services</h2><Button variant="outline" onClick={() => setContent({ ...content, services: [...content.services, { title: "New service", text: "Service description" }] })}><AdminIcon>+</AdminIcon> Add service</Button></div><div className="editor-cards">{content.services.map((item, index) => <article key={index}><div className="item-number">{String(index + 1).padStart(2, "0")}</div><Field label="Title" value={item.title} onChange={(v) => { const services = [...content.services]; services[index] = { ...item, title: v }; setContent({ ...content, services }); }} /><Field label="Description" value={item.text} multiline onChange={(v) => { const services = [...content.services]; services[index] = { ...item, text: v }; setContent({ ...content, services }); }} /><Button variant="ghost" onClick={() => setContent({ ...content, services: content.services.filter((_, i) => i !== index) })}><AdminIcon>x</AdminIcon> Remove</Button></article>)}</div></section></TabsContent>
      <TabsContent value="process"><section className="admin-card"><div className="card-heading"><h2>Working process</h2><Button variant="outline" onClick={() => setContent({ ...content, process: [...content.process, { title: "New step", text: "Step description" }] })}><AdminIcon>+</AdminIcon> Add step</Button></div><div className="editor-cards">{content.process.map((item, index) => <article key={index}><div className="item-number">{String(index + 1).padStart(2, "0")}</div><Field label="Step name" value={item.title} onChange={(v) => { const process = [...content.process]; process[index] = { ...item, title: v }; setContent({ ...content, process }); }} /><Field label="Description" value={item.text} multiline onChange={(v) => { const process = [...content.process]; process[index] = { ...item, text: v }; setContent({ ...content, process }); }} /><Button variant="ghost" onClick={() => setContent({ ...content, process: content.process.filter((_, i) => i !== index) })}><AdminIcon>x</AdminIcon> Remove</Button></article>)}</div></section></TabsContent>
      <TabsContent value="projects"><section className="admin-card"><div className="card-heading"><h2>Featured projects</h2><Button variant="outline" onClick={() => setContent({ ...content, projects: [...content.projects, { title: "New project", type: "Web design", url: "", imageUrl: "" }] })}><AdminIcon>+</AdminIcon> Add project</Button></div><div className="editor-cards project-editors">{content.projects.map((item, index) => <article key={index}><div className="item-number">{String(index + 1).padStart(2, "0")}</div>{item.imageUrl && <img src={item.imageUrl} alt="Project" />}<UploadButton onUploaded={(url) => { const projects = [...content.projects]; projects[index] = { ...item, imageUrl: url }; setContent({ ...content, projects }); }} /><Field label="Project name" value={item.title} onChange={(v) => { const projects = [...content.projects]; projects[index] = { ...item, title: v }; setContent({ ...content, projects }); }} /><Field label="Category / role" value={item.type} onChange={(v) => { const projects = [...content.projects]; projects[index] = { ...item, type: v }; setContent({ ...content, projects }); }} /><Field label="Project URL" value={item.url} placeholder="https://" onChange={(v) => { const projects = [...content.projects]; projects[index] = { ...item, url: v }; setContent({ ...content, projects }); }} /><Button variant="ghost" onClick={() => setContent({ ...content, projects: content.projects.filter((_, i) => i !== index) })}><AdminIcon>x</AdminIcon> Remove</Button></article>)}</div></section></TabsContent>
      <TabsContent value="tools"><section className="admin-card"><h2>Tools and contact links</h2><div className="admin-grid"><Field label="LinkedIn URL" value={content.links.linkedin} onChange={(v) => setContent({ ...content, links: { ...content.links, linkedin: v } })} /><Field label="GitHub URL" value={content.links.github} onChange={(v) => setContent({ ...content, links: { ...content.links, github: v } })} /><Field label="Public email" value={content.links.email} onChange={(v) => setContent({ ...content, links: { ...content.links, email: v } })} /><Field label="Footer tagline" value={content.footerTagline} onChange={(v) => setContent({ ...content, footerTagline: v })} /></div><h3>Tools and technologies</h3><div className="repeat-list">{content.tools.map((tool, index) => <div key={index}><Input value={tool} onChange={(e) => { const tools = [...content.tools]; tools[index] = e.target.value; setContent({ ...content, tools }); }} /><Button variant="ghost" size="icon" aria-label="Remove tool" onClick={() => setContent({ ...content, tools: content.tools.filter((_, i) => i !== index) })}><AdminIcon>x</AdminIcon></Button></div>)}</div><Button variant="outline" onClick={() => setContent({ ...content, tools: [...content.tools, "New tool"] })}><AdminIcon>+</AdminIcon> Add tool</Button></section></TabsContent>
    </Tabs>
    <div className="mobile-save"><Button onClick={save} disabled={saving}>{saving ? <AdminIcon spin>o</AdminIcon> : <AdminIcon>#</AdminIcon>} Save changes</Button></div>
  </main>;
}
